...
| 4 | `choosing-the-right-model` (decision wizard) | **Done — rebuilt + verified live** | Arun: "push it and complete both topics fully and push it. i will check tomorrow morning" — authorized completing the remaining pages without further per-page live checkpoints. | Removed the 2-question wizard and both `useState` hooks entirely. All 4 model recommendations (Opus/Sonnet/Haiku/Fable) now show simultaneously as cards with their real "when to use" reasoning from the transcript, color-matched to the model-family chart's palette for visual continuity across pages. Added a "common production pattern" section — a static triage flow diagram (Request → Haiku triage → simple/done or escalate to Sonnet/Opus) with animated dash-flow connector lines — surfacing a transcript sentence the old wizard never showed at all. **Verification bug found+fixed**: the page subtitle nearly duplicated the visual's own overview sentence verbatim — replaced with "Four models, each optimized for a different job." **Verified live 2026-07-22**: mobile/tablet/desktop all 0px overflow, 0 clickable elements, no runtime errors (the initial 24px desktop reading was a resize-timing artifact in the test harness, confirmed by a clean recheck without resizing). |
| 5 | `what-makes-claude-different` (flip cards) | **Done — rebuilt + verified live** | Same authorization as row 4. | Removed the click-to-flip mechanic and `useState` entirely. All 4 differentiator cards (safety-first training, long context windows, agentic tool use, artifacts) show their full explanation directly — no "click to flip →" prompt, no hidden back-face content. Page subtitle updated (old one described the flip mechanic). **Verification bug found+fixed**: same orphaned-card grid defect as `modes-of-interaction` (row 3) — `auto-fit minmax(280px)` with exactly 4 cards produced 3 columns + an orphan at desktop width. Fixed with the identical explicit-2-column media-query pattern. **Verified live 2026-07-22**: mobile/tablet/desktop all 0px overflow, 0 clickable elements, clean 2x2 grid on desktop, no runtime errors. |
...
| # | Page | Status | Arun's feedback | Action taken |
|---|------|--------|------------------|--------------|
| 6 | `why-oop` (why structure code this way) | **Done — built + verified live** | Covered by the CEO-approved scope decision + "complete both topics fully" authorization above. | No code excerpt (transcript has none for this chapter) — overview line, a "without structure vs. with objects" comparison pair, and a 3-card recap of what OOP buys you (reuse, enforced rules, shared vocabulary). **Verified live 2026-07-22**: mobile/tablet/desktop all 0px overflow (on fresh page loads — see note below), 0 clickable elements, no runtime errors. |
| 7 | `classes-and-objects` (blueprint vs. instance) | **Done — built + verified live** | Same. | Code excerpt: the `Car` class constructor + `drive()`. Callouts: `__init__` as constructor, `self` scoping per-instance state. Paired with a static visual showing `my_car`/`your_car` holding independently different `odometer` values — makes the "separate state per object" point concrete rather than just asserted. **Verified live 2026-07-22**: mobile/tablet/desktop all 0px overflow, 0 clickable elements, no runtime errors. |
| 8 | `encapsulation` (controlled access to state) | **Done — built + verified live** | Same. | Code excerpt: `BankAccount.deposit()` validating before mutating `_balance`. Callouts on the internal-by-convention attribute and validate-before-mutate pattern. Before/after card pair: what breaks without encapsulation (any code can set an invalid balance) vs. what encapsulation guarantees (`deposit()`/`withdraw()` are the only doors in). **Verified live 2026-07-22**: mobile/tablet/desktop all 0px overflow, 0 clickable elements, no runtime errors. |
| 9 | `abstraction` (interface vs. implementation) | **Done — built + verified live** | Same. | Code excerpt: `PaymentProcessor` abstract interface + `checkout()` depending only on it. Callouts on interface-stability. A diagram shows Stripe/PayPal as existing implementations and Apple Pay added as a new one — visually demonstrating a new provider slots in without touching `checkout()`. **Verified live 2026-07-22**: mobile/tablet/desktop all 0px overflow, 0 clickable elements, no runtime errors. |
| 10 | `inheritance` (shared base, specialized subclass) | **Done — built + verified live** | Same. | Code excerpt: `Animal.describe()`/`speak()` with `Dog` overriding `speak()`. Callouts on shared-vs-overridden methods. A small class tree diagram (Animal → Dog, Cat) shows each subclass's overridden `speak()` output. **Verified live 2026-07-22**: mobile/tablet/desktop all 0px overflow, 0 clickable elements, no runtime errors. |
| 11 | `polymorphism` (same call, per-type behavior) | **Done — built + verified live** | Same. | Code excerpt: `sum(shape.area() for shape in shapes)` with no type checks. Callouts on per-type correct behavior and the caller never checking concrete type. Before/after card pair: `if isinstance(...)` branch sprawl without polymorphism vs. zero-change extensibility with it. **Verified live 2026-07-22**: mobile/tablet/desktop all 0px overflow, 0 clickable elements, no runtime errors. |
| 12 | `oop-in-the-real-world` (the four pillars together) | **Done — built + verified live** | Same. | No code excerpt (transcript is a wrap-up, not new code) — a 4-card recap of each pillar's payoff, plus a "where you'll see it" list of real-world examples (web frameworks, game engines, enterprise systems) pulled directly from the transcript. The 4-pillar card row uses a smaller `minmax(200px)` than the 280px that caused the row-3/row-5 orphan bug, so all 4 fit one row cleanly with no fix needed. **Verified live 2026-07-22**: mobile/tablet/desktop all 0px overflow, 0 clickable elements, no runtime errors. |

**Note on measurement methodology**: while verifying `why-oop`, rapid resize-in-place across
multiple navigations within the same browser tab occasionally produced a stale/transient
`scrollHeight` reading (e.g. a 24px "overflow" that vanished on a fresh page load at the identical
viewport size, with the `FitToViewport` wrapper's own measured bounding box confirmed well within
budget). This was a test-harness artifact, not a shipped bug — the rest of this verification pass
used fresh navigations per viewport size to get reliable readings. Real users resizing their browser
window are covered by `FitToViewport`'s own `resize` event listener, which recalculates correctly
(already proven across many resize cycles throughout this session's manual QA).

**All 12 subtopics across both demo topics are now built, deployed, and verified live** — 0px scroll
overflow, 0 clickable elements, no runtime errors on every page at mobile/tablet/desktop. The
`/demo/oop-fundamentals` course page's Visuals tab was also confirmed to show all 7 working chapter
links (previously "No visuals for this demo course yet."). Two real bugs were found and fixed during
this pass: the `choosing-the-right-model` subtitle duplicating the visual's own overview line, and
the `what-makes-claude-different` 4-card grid producing an orphaned card on desktop (same class of
bug as `modes-of-interaction`, same fix). **Arun has not yet done his own visual/content review of
pages 4–12** — that's the next step when he checks in, per his "i will check tomorrow morning."
