...
- `@elevenlabs/client` — official ElevenLabs Conversational-AI browser client, 721k+ weekly downloads, latest 1.17.0, approved 2026-08-08 per B2B-75 (`docs/specs/B2B-75-requirement-document.md`). **Scoped to the widget channel only** — `lib/voice/elevenlabs-adapter.ts`, constructed only by `app/(with-clerk)/widget-render/[clio_session_ref]/WidgetRenderClient.tsx`. The inline / meeting-bot channel (`partner-render`) never touches it. **Pinned to the exact version `1.17.0` (no caret range), deliberately**: `lib/voice/elevenlabs-adapter.ts`'s tool-fault discriminator reads `context.clientToolName`, an undocumented field of the SDK's internal `BaseConversation.ts` rather than part of ElevenLabs' public contract, and the unit test for it can only assert our code against our own mock of that shape. Pinning means any bump past 1.17.0 is a deliberate, reviewed act rather than something that rides in on a routine lockfile update and silently degrades the discriminator (it fails safe — reverting to pre-B2B-75 behaviour — but silently). Two deliberate non-adoptions: `@elevenlabs/elevenlabs-js` (the server REST SDK) is **not** added, because `app/api/elevenlabs-token/route.ts` makes a single documented REST call and is a structural twin of the two existing `fetch()`-based token routes; `@elevenlabs/react`'s `useConversation()` hook is **not** added, because it would break the imperative adapter-class pattern all three providers share behind `VoiceSessionAdapter`. Availability flag: `ELEVENLABS_ADAPTER_AVAILABLE` in `lib/voice/provider-availability.ts`.
- `livekit-client` — official LiveKit SDK, actively maintained (2.21.0 published 2026-07-23), not deprecated. **We never install this directly.** It arrives **transitively**, as a direct runtime dependency of `@elevenlabs/client` (`"livekit-client": "2.16.1"` in that package's own `dependencies` block — not a `peerDependency`), supplying the WebRTC transport B2B-75 ships on. This entry exists despite being a no-action item today for one reason: **its version rides on ElevenLabs' release schedule, not ours**, so a future CVE or breaking change would reach this codebase through a dependency nobody here chose directly — and the next person should find that already written down rather than rediscover it mid-incident.
- `@upstash/redis` — official Upstash Redis SDK, approved 2026-08-01 per B2B-63 (live transcript capture for OpenAI Realtime sessions, since OpenAI has no post-hoc transcript API — see `docs/specs/B2B-63-requirement-document.md`). Provisioned via the Vercel Marketplace Upstash integration specifically (Arun's direct preference for one consolidated bill, not a separate vendor). Accessed via ordinary REST API calls (`Redis.fromEnv()`), unlike the voice adapters' deliberate raw-WebSocket approach — there is no structural reason to avoid a typed SDK here, so the general "typed SDK client, never raw fetch to third-party endpoints" rule applies normally. First Redis/cache dependency in this codebase; `lib/partner/rate-limit.ts`'s standing comment anticipated this but this approval is scoped to B2B-63's transcript-capture use only — reusing it for rate limiting is a separate, not-yet-made decision.
- `gsap` + `@gsap/react` — official GreenSock animation library and its official React integration hook (`useGSAP`, for StrictMode-safe context cleanup), approved 2026-08-12 per Arun's direct instruction for the marketing homepage rewrite ("technological innovative WOW factor" hero visuals, used throughout the page, alongside `framer-motion`). 100% free for commercial use since April 2025 (Webflow acquired GreenSock and released every formerly-paid Club plugin — ScrollTrigger, SplitText, MorphSVG, DrawSVG, ScrollSmoother — at no charge), millions of weekly downloads, actively maintained (gsap 3.15.0). Scoped to marketing/landing-page visuals; `framer-motion` remains the default for ordinary component-level transitions elsewhere in the app — this doesn't replace it.
...
- `@11labs/client`, `elevenlabs` — replaced by Hume EVI as the sole voice provider, 2026-07-13 (direct owner instruction, not a toggle — full removal in progress). **These two package names remain removed and must not be used.** `@11labs/client` is additionally deprecated upstream ("This package is no longer maintained. Please use @elevenlabs/client for the latest version.").
  **Status amended 2026-08-08 (B2B-75):** the *vendor* is back, for the widget channel only, under its successor package name `@elevenlabs/client` (see the approved list above). This is a deliberate re-introduction of ElevenLabs as a selectable widget voice provider — **not** a revival of the 2026-07-13 removal's architecture: no agent pool (`lib/elevenlabs-pool.ts`, deleted in 7a0020a), no server-side audio relay (`server.ts` / `lib/voice/relay-handler.ts`, deleted), and no `NEXT_PUBLIC_VOICE_PROVIDER` env toggle — the provider is chosen from the admin dashboard, per channel, and credentials are stored encrypted in the database rather than in env vars. The removal history above stays on the record; only its status changed.
