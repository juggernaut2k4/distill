...
function StepNode({ data }: NodeProps) {
  ...
  return (
    <div className="rounded-xl border border-[color-mix(in_srgb,var(--partner-primary,#7C3AED)_40%,transparent)] bg-[#111111] p-4 min-w-[260px] max-w-[340px]">
      <Handle type="target" position={Position.Left} style={{ background: 'var(--partner-primary, #7C3AED)', border: 'none' }} />
      <Handle type="source" position={Position.Right} style={{ background: 'var(--partner-primary, #7C3AED)', border: 'none' }} />
      <div className="flex items-start gap-3">
        <div className="shrink-0 w-8 h-8 rounded-full bg-[var(--partner-primary,#7C3AED)] flex items-center justify-center text-white font-bold text-sm">{d.number}</div>
        <div className="flex-1">
          ...
          {d.what_to_watch_for && (
            <div className="mt-2 text-sm text-[var(--partner-accent,#F59E0B)] bg-[color-mix(in_srgb,var(--partner-accent,#F59E0B)_10%,transparent)] rounded px-2 py-1">
              ⚠ {d.what_to_watch_for}
            </div>
          )}
        </div>
