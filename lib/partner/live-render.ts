...
/** Which channel's rule set `buildInlineSessionContent()`'s per-page stage direction should speak
 *  to. Defaults to `'meeting_bot'` so every existing caller's output stays byte-identical. */
export type InlineStageDirectionVariant = 'meeting_bot' | 'widget'
 ...
 *
 * 2026-08-07 — this function's stage-direction text is shared between the meeting-bot channel
 * (`PartnerRenderClient.tsx`, this file's original caller) and the widget channel
 * (`app/(with-clerk)/widget-render/[clio_session_ref]/page.tsx`), but the two channels' prompts
 * disagree on rule numbers, tool-calling order, and whether a fixed wrap-up phrase should be
 * spoken at all — the widget's own `widget-prompt-rules.ts` v21 rewrite found the shared text was
 * pointing widget sessions at a nonexistent "rule 8," ordering `advance_tab` backwards relative to
 * its own rule 3h, and mandating a preamble its own G16 bans. The `variant` param (default
 * `'meeting_bot'`, reproducing this function's pre-2026-08-07 output byte-for-byte) lets each
 * channel get its own correct stage direction from one shared body, rather than forking the whole
 * function — only the two branches below actually differ per channel; everything else (title,
 * subtitle, page headers, content blocks) is genuine shared formatting.
 */
export function buildInlineSessionContent(
  session: PartnerSessionRow,
  pages: InlineContentPage[],
  variant: InlineStageDirectionVariant = 'meeting_bot',
): string {
  const blocks: string[] = []
  ...
  // B2B-60 §5 — non-blocking collision observability, meeting_bot variant only: the widget prompt
  // no longer asks the model to speak STAGE_1_WRAP_UP_PHRASE at all (see the widget branch below),
  // so checking widget sessions against it would only ever produce a meaningless warning. Same
  // scope of source text as the pattern computeStage2Eligibility uses client-side (session-level
  // narration + every page's own narration content), checked once here against the fixed Stage 1
  // phrase. Never blocks session creation — there is no fallback phrase to regenerate into (the
  // phrase is fixed and global by design).
  if (variant === 'meeting_bot') {
  ...
  pages.forEach((page, index) => {
    ...
    if (variant === 'widget') {
      // 2026-08-07 — states only per-page facts (page number, this page's closing point, the next
      // page's title). Zero speaking or tool-calling instructions: widget-prompt-rules.ts's own
      // rules 3/6 own all of that now, so there is nothing here to contradict them.
      lines.push(
        isLast
          ? `[FOR YOU ONLY — DO NOT SAY] Page ${pageNo} of ${pages.length}, the last one. Closing point for this page: "${page.transition_trigger}". There is no next page — after this one, go to rule 6.`
          : `[FOR YOU ONLY — DO NOT SAY] Page ${pageNo} of ${pages.length}. Closing point for this page: "${page.transition_trigger}".${pages[index + 1]?.title ? ` Next page: "${pages[index + 1].title}".` : ''}`
      )
    } else if (isLast) {
