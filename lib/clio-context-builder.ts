/**
 * Builds Clio's three session documents — combined into the system prompt served
 * by the Hume Custom-LLM bridge (app/api/clio/chat/completions/route.ts).
 *
 ...
 *
 * All three are stored in walkthrough_state and combined into the system prompt
 * fetched by that Custom-LLM endpoint on every conversation turn.
 */
...
// producing byte-identical output for its existing Hume-Custom-LLM/
...
export interface ClioSessionDocs {
  ...
  /** Combined system prompt served by the Hume Custom-LLM bridge */
