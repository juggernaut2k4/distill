...
export const BOT_ELEVENLABS_PROMPT_VERSION = 'bot-el-v1'

// ─── Placeholders ────────────────────────────────────────────────────────────────────────────────

export const BOT_ELEVENLABS_CONTEXT_PLACEHOLDER = '[CONTEXT]'
export const BOT_ELEVENLABS_SESSION_CONTENT_PLACEHOLDER = '[SESSION_CONTENT]'
export const BOT_ELEVENLABS_TONE_GUIDANCE_PLACEHOLDER = '[TONE_GUIDANCE]'
export const BOT_ELEVENLABS_PARTNER_GUIDANCE_PLACEHOLDER = '[PARTNER_CONFIGURED_GUIDANCE]'
export const BOT_ELEVENLABS_AUDIENCE_PLACEHOLDER = '[AUDIENCE]'
export const BOT_ELEVENLABS_BOT_NAME_PLACEHOLDER = '[BOT_NAME]'
export const BOT_ELEVENLABS_PARTICIPANT_NAME_PLACEHOLDER = '[PARTICIPANT_NAME]'
export const BOT_ELEVENLABS_INDUSTRY_CLAUSE_PLACEHOLDER = '[INDUSTRY_CLAUSE]'
export const BOT_ELEVENLABS_LANGUAGE_INSTRUCTION_PLACEHOLDER = '[LANGUAGE_INSTRUCTION]'
export const BOT_ELEVENLABS_ADAPTIVE_DELIVERY_PLACEHOLDER = '[ADAPTIVE_DELIVERY_GUIDANCE]'

export type BotElevenLabsPromptFieldMode = 'literal' | 'instruction'

export interface BotElevenLabsDualModePromptField {
  mode: BotElevenLabsPromptFieldMode
  text: string
}
...
export interface BotElevenLabsPromptBehaviorConfig {
  tonePersona?: BotElevenLabsDualModePromptField | null
  deferralPhrasing?: BotElevenLabsDualModePromptField | null
  closingConfirmationQuestion?: BotElevenLabsDualModePromptField | null
  goodbyeLine?: BotElevenLabsDualModePromptField | null
  verificationQuestionStyle?: string | null
  interSectionRecapStyle?: string | null
}

export const BOT_ELEVENLABS_PROMPT_TEMPLATE = `You are ${BOT_ELEVENLABS_BOT_NAME_PLACEHOLDER}, an AI coach delivering a live, one-on-one voice coaching session to ${BOT_ELEVENLABS_AUDIENCE_PLACEHOLDER}${BOT_ELEVENLABS_INDUSTRY_CLAUSE_PLACEHOLDER}. Be a trusted mentor: warm, calm, confidently knowledgeable, and conversational — never a hyped-up coach, never a script being read aloud.${BOT_ELEVENLABS_TONE_GUIDANCE_PLACEHOLDER}${BOT_ELEVENLABS_LANGUAGE_INSTRUCTION_PLACEHOLDER}
...
G11. Never read SESSION CONTENT aloud as written. Teach it in your own words.${BOT_ELEVENLABS_ADAPTIVE_DELIVERY_PLACEHOLDER}
...
1. Opening.
1a. Greet ${BOT_ELEVENLABS_PARTICIPANT_NAME_PLACEHOLDER} and introduce yourself.
 ...
 */
export interface AssembleBotElevenLabsPromptInput {
  profileContext: string
  intentContext: string
  sessionContent: string
  assistantName?: string
  promptBehavior?: BotElevenLabsPromptBehaviorConfig | null
...
function renderDualField(label: string, ruleRef: string, field: BotElevenLabsDualModePromptField): string {
...
function buildToneGuidance(field: BotElevenLabsDualModePromptField | null | undefined): string {
  if (!field) return ''
...
function buildPartnerGuidanceBlock(cfg: BotElevenLabsPromptBehaviorConfig | null | undefined): string {
  if (!cfg) return ''
  const parts: string[] = []
 ...
 */
export function assembleBotElevenLabsPrompt(input: AssembleBotElevenLabsPromptInput): string {
  const {
    profileContext,
    intentContext,
    sessionContent,
  ...
  return BOT_ELEVENLABS_PROMPT_TEMPLATE
    .split(BOT_ELEVENLABS_BOT_NAME_PLACEHOLDER).join(resolvedAssistantName)
    .split(BOT_ELEVENLABS_TONE_GUIDANCE_PLACEHOLDER).join(toneGuidance)
    .split(BOT_ELEVENLABS_LANGUAGE_INSTRUCTION_PLACEHOLDER).join(languageInstruction)
    .split(BOT_ELEVENLABS_ADAPTIVE_DELIVERY_PLACEHOLDER).join(buildAdaptiveDeliveryGuidance())
    .split(BOT_ELEVENLABS_PARTNER_GUIDANCE_PLACEHOLDER).join(partnerGuidance)
    .split(BOT_ELEVENLABS_PARTICIPANT_NAME_PLACEHOLDER).join(resolvedParticipantName)
    .split(BOT_ELEVENLABS_INDUSTRY_CLAUSE_PLACEHOLDER).join(industryClause)
    .split(BOT_ELEVENLABS_AUDIENCE_PLACEHOLDER).join(audienceDescription)
    .split(BOT_ELEVENLABS_CONTEXT_PLACEHOLDER).join(contextBlock || '(No prior profile or intent data available yet — this is the participant\'s first session.)')
    .split(BOT_ELEVENLABS_SESSION_CONTENT_PLACEHOLDER).join(sessionContent ?? '')
}
