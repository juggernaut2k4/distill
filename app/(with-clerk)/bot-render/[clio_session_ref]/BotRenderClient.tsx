...
import { resolveBotJumpIndex, computeNextProgressIndex } from '@/lib/voice/bot-jump-resolution'
import { createJumpGuardState, shouldAllowJump } from '@/lib/partner/bot-jump-debounce'

/**
 * Production bot-render channel's render + tool-handling implementation (2026-08-12).
 *
 * A byte-for-byte fork of `widget-render/[clio_session_ref]/WidgetRenderClient.tsx` — the demo's
 * proven implementation — per Arun's explicit instruction: production gets its own copy of
 * everything so no production change can ever accidentally impact the demo/widget-sessions path,
 * even by accident. The core prompting, ElevenLabs configuration, and tool-calling behavior are
 * preserved EXACTLY as the demo's — only the Hume and OpenAI Realtime provider branches are removed
 * (production is ElevenLabs-only, per Arun's direct instruction), which is a pure subtraction of
 * unused code paths, not a behavior change to the ElevenLabs path itself. The original
 * `WidgetRenderClient.tsx` is untouched and continues to serve the demo independently.
 */
...
export interface BotInlinePageProp {
...
export interface BotRenderClientProps {
  clioSessionRef: string
  inlinePages: BotInlinePageProp[]
  // Production bot-render is ElevenLabs-only, per Arun's direct instruction (2026-08-12) — no
  // Hume/OpenAI Realtime provider branching. The agent id (a plain identifier, NOT a secret; the
  // API key is never a prop and never reaches the browser) and this channel's own ElevenLabs
  // prompt, assembled server-side by lib/voice/bot-elevenlabs-prompt-rules.ts.
...
// 2026-08-09 — uses the native end_call system tool instead of the custom end_session client tool
// (per Arun's direct instruction to switch to it). end_call's own `message` parameter is spoken by
// the ElevenLabs platform itself before hangup, so this asks Clio to carry the goodbye as that
// parameter rather than needing to say it out loud first, then call the tool in a separate step —
// see bot-elevenlabs-prompt-rules.ts's G22 for the mirrored, primary copy of this instruction (this
// nudge is the one client-side path that can't go through that file, since it fires from a plain JS
// timer, not from the prompt).
...
export default function BotRenderClient({
  clioSessionRef,
  inlinePages,
  elevenlabsAgentId,
  elevenlabsVoiceInstructions,
}: BotRenderClientProps) {
  const count = inlinePages.length

  const voiceEnabled = Boolean(elevenlabsAgentId)
      ...
      try {
        ...
        const tools = {
          show_visual: async (params: Record<string, unknown>) => {
            ...
            const idx = resolveBotJumpIndex(params, inlinePages, displayedIndexRef.current)
            // 2026-08-09 — free-navigation redesign, per Arun's direct instruction: the prompt
            // (bot-elevenlabs-prompt-rules.ts) never calls advance_tab, so show_visual must carry
            // advance_tab's progress-tracking job itself. Calling it for the exact next page in
            // SESSION CONTENT order IS what "moving forward" means; calling it for any other page
            // (jumping elsewhere to answer a question, or returning to the page already at the front
            // of progress) only changes what's displayed — mirrors computeNextProgressIndex's own
            // forward-only clamp, just triggered from this tool instead of a separate one.
            if (idx === progressIndexRef.current + 1) {
          ...
          advance_tab: async () => {
            // 2026-08-09 — code-level guard, per Arun's explicit instruction: the prompt
            // (bot-elevenlabs-prompt-rules.ts) never instructs calling this tool (show_visual alone
            // carries progress — see its own handler above), but that is only a prompt-level
            ...
            // disrupts the call — only the one stray tool call itself is rejected.
        ...
        // B2B-75 §6.7c. No `mediaStream` is passed: the @elevenlabs/client SDK captures and streams
        // microphone audio itself once permission has been granted. `micStream` above is still
        // obtained (it must run to get permission before startSession, and it feeds the mic
        // analyser tap) — it is simply not handed to this adapter.
        // 2026-08-10 (migration 113) — clio_session_ref lets the token route resolve this session's
        // own elevenlabs_agent_id, falling back to the system-wide default when the session has
        // none set.
