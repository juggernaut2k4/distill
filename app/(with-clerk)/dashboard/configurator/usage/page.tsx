...
import UsageLogClient from './UsageLogClient'

/**
 * /dashboard/configurator/usage — B2B-57b Requirement Doc §3/§6.5, mirrors
 * `app/(with-clerk)/dashboard/configurator/known-bugs/page.tsx`'s exact shape: Clerk `auth()` gate,
 * `getConfiguratorAccountsForClerkUser`, `<NoPartnerAccounts />` if empty, the standard wizard
 ...
 */
export default async function UsagePage({ searchParams }: { searchParams: { partner_account_id?: string } }) {
  const { userId } = auth()
  ...
  return <UsageLogClient accounts={accounts} activePartnerAccountId={activeId} billingHealth={billingHealth} />
}
