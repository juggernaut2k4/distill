...
/**
 * /dashboard/configurator/topics — B2B-20 §9, §12. The standalone section route is
 * retired; the section is now hosted inside the unified left-nav surface. This
 * page redirects into `/dashboard/configurator?section=topics`, preserving old deep
 * links and `partner_account_id`.
 */
export default async function TopicsRedirectPage({ searchParams }: { searchParams: { partner_account_id?: string } }) {
  const { userId } = auth()
  ...
  redirect(`/dashboard/configurator?partner_account_id=${activeId}&section=topics`)
}
